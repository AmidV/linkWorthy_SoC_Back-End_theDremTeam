...
export async function addANoteToTheWeek(week, tags, summary, link, isComplete) {
  const data = await query(
    `INSERT INTO information ( week, tags, summary, link, isComplete ) VALUES ( $1, $2, $3, $4, $5 );`,
...
export async function updateTags(id, tags) {
  const data = await query(`UPDATE information SET tags = $1 WHERE id = $2;`, [
    id,
    tags,
  ]);
  return data.rows;
...
export async function updateSummary(id, summary) {
  const data = await query(
    `UPDATE information SET summary = $1 WHERE id = $2;`,
    [id, summary]
  );
  return data.rows;
}
...
export async function updateLink(id, link) {
  const data = await query(`UPDATE information SET link = $1 WHERE id = $2;`, [
    id,
    link,
  ]);
  return data.rows;
...
export async function getIsComplete(id, isComplete) {
  const data = await query(
    `UPDATE information SET isComplete = $1 WHERE id = $2;`,
    [id, isComplete]
  );
  return data.rows;
}
